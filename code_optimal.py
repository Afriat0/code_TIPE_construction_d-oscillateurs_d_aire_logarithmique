...
Ces dernieres sont:
    ...
    Le syringe, qui permet de transformer un glider en Herschel, un H_to_G qui transforme un herschel en glider, une figure sur mesure appelée Mega_bistable ainsi que gadget sont utilisées.
...
gadget_off=repertoire.gadget_off
gadget_on=repertoire.gadget_on
gadget_renverse_off=repertoire.gadget_renverse_off
gadget_renverse_on=repertoire.gadget_renverse_on

h_to_g=repertoire.h_to_g
syringe=repertoire.syringe

mega_bistable=repertoire.mega_bistable
mega_bistable_decale=repertoire.mega_bistable_decale
...
-La clé "dx,dy" correspond au déplacement dans la grille du haut gauche du herschel ou du planeur en entrée. x est l'axe vertical, y l'axe horizontal de sorte que l'on accéde à l'etat de l en (x,y) par l'appel l[x][y]
-La clé "rotation" correspond a la rotation
-La clé "positionne" correspond a la position relative du haut gauche du conduit par rapport au haut gauche du Herschel ou du planeur
-La clé "dimension_de_l'entrée" donne la dimension de la figure en entrée du conduit. Elle correspond a (4,3) pour Herschel mais cette representation se veut generale, par exemple en incluant Snark. SemiSnark est naivement considére comme un conduit tout comme le eater
...
conduitgadget_off={"droit":gadget_off,"dx,dy":(59,0),"rotation":"droit","positionne":(18,-12),"dimension_de_l'entree":(3,3),"emission":[],"ticks":378}
conduitgadget_renverse_off={"droit":gadget_renverse_off,"dx,dy":(0,59),"rotation":"droit","positionne":(-11,18),"dimension_de_l'entree":(3,3),"emission":[],"ticks":378}

conduitgadget_on={"droit":gadget_on,"dx,dy":(59,0),"rotation":"droit","positionne":(18,-22),"dimension_de_l'entree":(3,3),"emission":[("glider","gauche",395,(39,-14))],"ticks":378}
conduitgadget_renverse_on={"droit":gadget_renverse_on,"dx,dy":(0,59),"rotation":"droit","positionne":(-21,18),"dimension_de_l'entree":(3,3),"emission":[("glider","gauche",395,(39,-14))],"ticks":378}
...
conduitmega_bistable_decale={"droit":mega_bistable_decale,"dx,dy":(140,140),"rotation":"droit","positionne":(13,-4),"dimension_de_l'entree":(3,3),"emission":[],"ticks":560,"repeat time":(76,104),"ajout":1042}
...
conduitmega_bistable_decale_sans_sortie={"droit":mega_bistable_decale_sans_sortie,"dx,dy":(140,140),"rotation":"droit","positionne":(13,-4),"dimension_de_l'entree":(3,3),"emission":[],"ticks":560,"repeat time":(76,104),"ajout":1042}
...
def place_une_figure(l,fig,haut_gauche,r):
    if r=="droit":
...
"""positionner_un_conduit prend en entrée une grille l, la position du haut gauche d'une figure ainsi que son orientation representée par un string r, et un conduit qui prend pour entrée cette figure, et modifie l en ajoutant ce conduit afin qu'il agisse sur la figure placée en haut_gauche et d'orientation r"""

def positionner_un_conduit(l,haut_gauche,r,conduit):
...
"""positionner_puis_faire_avancer_une_figure prend en entrée une grille l, la position du haut gauche d'une figure ainsi que son orientation representée par un string r, et un conduit qui prend pour entrée cette figure, et modifie l en ajoutant ce conduit afin qu'il agissent sur la figure placée en haut_gauche et d'orientation r, puis renvoie l'emplacement et l'orientation de la figure apres sa modification par le conduit"""


def positionner_puis_faire_avancer_une_figure(l,haut_gauche,r,conduit):
    positionner_un_conduit(l,haut_gauche,r,conduit)
    a,b=conduit["dimension_de_l'entree"]
    c,d=a,b
...
"""deplace_un_planeur prend en entree une position haut gauche, une orientation d'un glider et un entier d, et renvoie la position du haut_gauche du glider apres 4*d ticks"""


def deplace_un_planeur(haut_gauche,r,d):
    if r=="droit":
...
def taille_de_oscillateur_de_constantes(k1:int,k2:int,k3:int,A:int,B:int)->(int,int):
...
"""toutes_les_cellules_voisines_d_une_liste prend en entrée la liste des coordonnées d'une partie M de la grille
...
on peut savoir grace a toutes_les_cellules_voisines_d_une_liste quelles cellules sont succeptibles de changer d'état
au prochain tick."""

def toutes_les_cellules_voisines_d_une_liste(l1:list[int,int])->list[int,int]:
...
"""etat_suivant_de_la_grille prend en entrée une grille et une liste des coordonnées des cellules
qui sont succeptibles d'etre modifiées au prochain tick, et fait évoluer la grille d'un tick,
et la renvoie. si ind=True, il renvoie aussi la liste des coordonnées des cellules
ayant été modifiées"""


def etat_suivant_de_la_grille(l,l1=[[]],ind=False)->int:
    n,m=len(l),len(l[0])
    if l1==[[]]:
    ...
    l2=[]
    for i in range(len(l1)) :
        t[i]=etape_du_jeu(l,l1[i][0],l1[i][1])
...
remplir_le_circuit_avec_a_herschels prend en entrée une grille l contenant un circuit de herschel encore vide,
...
l'idée de remplir_le_circuit_avec_a_herschels est de rajouter un Herschel au début du circuit que l'on a
...
def remplir_le_circuit_avec_a_herschels(l:list[list[int]],p:int,a:int,haut_gauche:(int,int))->None:
    h=haut_gauche
    ...
    for d in range(a-1):
        change=[]
        for c in range(p):
            bouge=toutes_les_cellules_voisines_d_une_liste(bouge)
            l1,bouge=etat_suivant_de_la_grille(l1,bouge,True)
            for i in bouge:
                if i not in change:
                    change.append(i)
        for i in change:
...
def choix_preferentiel_du_prohain_conduit_Forward(l,haut_gauche,r,a154,a116,a117):
    c=True
    if c and a117>0:
        haut_gauche,r=positionner_puis_faire_avancer_une_figure(l,haut_gauche,r,conduit117)
        c=False
        a117-=1
    if c and a154>0:
        haut_gauche,r=positionner_puis_faire_avancer_une_figure(l,haut_gauche,r,conduit154)
        a154-=1
        c=False
    if c and a116>0:
        haut_gauche,r=positionner_puis_faire_avancer_une_figure(l,haut_gauche,r,conduit116)
        a116-=1
        c=False
    return haut_gauche,a154,a116,a117

"""renvoie l'oscillateur naif associé a k1,k2,k3,A et B ainsi que a. les conduits sont ajoutés en suivant les règles de créer_liste_pour_oscillateur_naif"""

def oscillateur_de_constantes(p,a,k1,k2,k3,A,B):
    if p>=61:
        taillemax=taille_de_oscillateur_de_constantes(k1,k2,k3,A,B)
        l=[]
        ...
        for x in range(2):
            ...
            a190-=b190
            for k in range(a64):
                haut_gauche,r=positionner_puis_faire_avancer_une_figure(l,haut_gauche,r,conduit64)
                haut_gauche,a154,a116,a117=choix_preferentiel_du_prohain_conduit_Forward(l,haut_gauche,r,a154,a116,a117)
                haut_gauche,r=positionner_puis_faire_avancer_une_figure(l,haut_gauche,r,conduit112)
                haut_gauche,a154,a116,a117=choix_preferentiel_du_prohain_conduit_Forward(l,haut_gauche,r,a154,a116,a117)
            for k in range(a190):
                haut_gauche,r=positionner_puis_faire_avancer_une_figure(l,haut_gauche,r,conduit190)
                haut_gauche,a154,a116,a117=choix_preferentiel_du_prohain_conduit_Forward(l,haut_gauche,r,a154,a116,a117)
                haut_gauche,r=positionner_puis_faire_avancer_une_figure(l,haut_gauche,r,conduit112)
                haut_gauche,a154,a116,a117=choix_preferentiel_du_prohain_conduit_Forward(l,haut_gauche,r,a154,a116,a117)
            if b64>=1:
            ...
            if b64==2:
                ...
                haut_gauche,r=positionner_puis_faire_avancer_une_figure(l,haut_gauche,r,conduit190)
        if a>2:
            remplir_le_circuit_avec_a_herschels(l,p,a,(50,50))
        return l
    else:
        pass


""" creer_liste_pour_oscillateur_naif prend en entrée k1,k2,k3,A,et B et renvoie une liste contenant 154,116,117,64 et 192 ainsi que 112 selon les regles:
...
def creer_liste_pour_oscillateur_naif(k1,k2,k3,A,B):
    if k1>0:
        liste=[154]
        k1-=1
    def f(l,k1,k2,k3):
...
"""verifier_si_a_est_solution_exhaustif renvoie (k1,k2,k3,A,B) tels que pa=2*(k1*154+k2*116+k3*113+A*64+B*192+(A+B-2)*112) s'ils existent et False sinon. On impose:
...
On remarque son execution se fait en O(1) si k2 et k3 sont bornés, c'est à dire si il existe une solution telle que k2 et k3 sont considérés bornés"""

def verifier_si_a_est_solution_exhaustif(periode:int,pa:int):
    p=pa//2
                ...
                if periode>=63 or k3==0:
                    for A in range(3):
                        ...
                        if periode>=107 or B==0:
                            k1=(pa-2*(k2*116+k3*117+A*64+B*190))/(2*154)
                            if k1+k2+k3>=2 and k1%1==0 and k1>0:
                                return (int(k1),k2,k3,A,B)
    return False

"""trouve_les_plus_petites_constantes_exhaustif prend en entrée p un entier et renvoie a,k1,k2,k3,A,B tels que verifier_si_a_est_solution_exhaustif(p,p*a) trouve k1,k2,k,A et B, avec a minimal. Comme empiriquement pour p assez grand, il existe une solution pour a=1+p%2 et que k2 et k3 sont bornés, son execution se fait en O(1)"""

def trouve_les_plus_petites_constantes_exhaustif(p:int)->(int,int,int,int,int,int):
    a=p%2
    verite=True
    while verite:
        a+=1
        x=verifier_si_a_est_solution_exhaustif(p,p*a)
        if x!= False:
            k1,k2,k3,A,B=x
            return (a,k1,k2,k3,A,B)


"""oscillateur_naif prend en entree un entier p et renvoie une grille contenant un oscillateur de periode p, selon la methode naive"""

def oscillateur_naif(p):
    a,k1,k2,k3,A,B=trouve_les_plus_petites_constantes_exhaustif(p)
    return oscillateur_de_constantes(p,a,k1,k2,k3,A,B)

"""canon_naif prend en entrée un entier p et renvoie une grille contenant un canon de période p, obtenu en appelant oscillateur_naif(p) puis en retirant le eater du premier F154, placé au début car k1>=1"""

def canon_naif(p):
    if p>=69:
        l=oscillateur_naif(p)
...
def canon_booste_puissance_2(p,ind=False):
    ...
    for i in range(m):
        if r=="a l'envers":
            haut_gauche,r=positionner_puis_faire_avancer_une_figure(l,haut_gauche,r,conduitsemisnarkrenverseactif)
        elif r=="gauche":
            haut_gauche,r=positionner_puis_faire_avancer_une_figure(l,haut_gauche,r,conduitsemisnarkactif)
    if ind:
...
def positionner_a_la_suite_les_conduits(haut_gauche,liste,l,r):
    for x,d in liste:
        haut_gauche=deplace_un_planeur(haut_gauche,r,d)
        haut_gauche,r=positionner_puis_faire_avancer_une_figure(l, haut_gauche,r,liste_de_conduit[x])
...
def auxiliaire_de_booste_puissance_2_compact(m,y):
    lemax=(y-100)//(17)
...
"""ces deux fonctions font exactement la même chose que les deux fonctions précedentes canon et oscillateur, à cela prés que les Semi_Snark sont positionés de manière plus compacte visuelement"""
...
def canon_booste_puissance_2_compact(p,ind=False,ind2=False):
    ...
    liste,lemax=auxiliaire_de_booste_puissance_2_compact(m,len(l[0]))
    x=m//lemax+1
    l=[[0 for i in range(len(l[0]))] for j in range(70*x)]+l
    haut_gauche,r=(25+70*x,55),"a l'envers"
    haut_gauche,r=positionner_a_la_suite_les_conduits(haut_gauche,liste,l,r)
    if ind:
        return l,haut_gauche,r
    return l
...
"""place_etape_de_bit_pour_multiplicateur_de_periode prend en entrée une grille, une position de glider, une orientation r, un bit x et une distance d valant par défaut 0 et modifie l en y ajoutant 1 Snark et 1 SemiSnark, activé si x=0 et désactivé si x=0. la distance entre le Snark et le SemiSnark est augmentée de d. le nouveau haut_gauche et r sont renvoyés."""

def place_etape_de_bit_pour_multiplicateur_de_periode(l,haut_gauche,r,x,d=0):
    haut_gauche=deplace_un_planeur(haut_gauche,r,17)
    haut_gauche,r=positionner_puis_faire_avancer_une_figure(l,haut_gauche,r,conduitsnarkrenverse)
    haut_gauche=deplace_un_planeur(haut_gauche,r,d+10)
    if x==1:
        haut_gauche,r=positionner_puis_faire_avancer_une_figure(l,haut_gauche,r,conduitsemisnark)
    if x==0:
        haut_gauche,r=positionner_puis_faire_avancer_une_figure(l,haut_gauche,r,conduitsemisnarkactif)
    haut_gauche=deplace_un_planeur(haut_gauche,r,-6)
    return haut_gauche,r

"""fin_de_bit_pour_multiplicateur_de_periode prend en entrée une grille l, la position d'un glider, son orientaion, une liste de bit et une distance d, par défaut valant 0, et modifie l pour terminer le period multiplier, en effectuant une rotaion puis en ajoutant les Gadgets pour chaque x dans liste, 1 pour off et 0 pour on."""

def fin_de_bit_pour_multiplicateur_de_periode(l,haut_gauche,r,liste,d=0):
    haut_gauche=deplace_un_planeur(haut_gauche,r,8)
    haut_gauche,r=positionner_puis_faire_avancer_une_figure(l,haut_gauche,r,conduitsnark)
    haut_gauche=deplace_un_planeur(haut_gauche,r,2+d)
    for i in liste:
        if i==1:
            haut_gauche,r=positionner_puis_faire_avancer_une_figure(l,haut_gauche,r,conduitgadget_renverse_off)
        if i==0:
            haut_gauche,r=positionner_puis_faire_avancer_une_figure(l,haut_gauche,r,conduitgadget_renverse_on)
    return haut_gauche,r

"""multiplicateur_de_periode prend en entrée une grille l, la position d'un glider, son orientaion, un entier n et une distance d, par défaut valant 0, et modifie l en ajoutant un n-multiplicateur de période pour glider. en effet, si on écrit x en base 2 et qu'on utilise des Semi Snark off pour les 1 et des SemiSnark on pour les 0, en les chainant pour écrire x en base 2 on bloque exactement x gliders avant d'en laisser passer 1, soit un (x+1) multiplicateur à usage unique, et les Gadgets permettent de maintenir cet arrangement """

def multiplicateur_de_periode(l,haut_gauche,r,n,d=0):
    haut_gauche=deplace_un_planeur(haut_gauche,r,15)
    if n==1:
        liste=[0]
    else:
        liste=ecriture_binaire(n-1)
    for i in range(len(liste)-1):
        i=len(liste)-i-1
        haut_gauche,r=place_etape_de_bit_pour_multiplicateur_de_periode(l,haut_gauche,r,liste[i])
    haut_gauche,r=place_etape_de_bit_pour_multiplicateur_de_periode(l,haut_gauche,r,liste[0],d)
    haut_gauche=deplace_un_planeur(haut_gauche,r,1)
    haut_gauche,r=fin_de_bit_pour_multiplicateur_de_periode(l,haut_gauche,r,liste,d)
    return haut_gauche,r

"""multiplicateur_de_periode_active fait de même, mais tout les Semi_Snark sont initalement activés"""

def multiplicateur_de_periode_active(l,haut_gauche,r,n,d=0):
    haut_gauche=deplace_un_planeur(haut_gauche,r,15)
    if n==1:
        liste=[0]
    else:
    ...
    for i in range(len(liste)-1):
        i=len(liste)-i-1
        haut_gauche,r=place_etape_de_bit_pour_multiplicateur_de_periode(l,haut_gauche,r,0)
    haut_gauche,r=place_etape_de_bit_pour_multiplicateur_de_periode(l,haut_gauche,r,0,d)
    haut_gauche=deplace_un_planeur(haut_gauche,r,1)
    haut_gauche,r=fin_de_bit_pour_multiplicateur_de_periode(l,haut_gauche,r,liste,d)
    return haut_gauche,r

"""check_qu_il_n_y_a_pas_de_recouvrement et cherche_d_evitant_un_recouvrement_exhaustif ont pour but de vérifier que les gliders parcourant dans les deux sens le period multiplier n'interferent pas, en renvoyant un entier d tel qu'en ajoutant un décalage de 8*d par un deplacement de d cases des Snark, on évite toute collision"""

def check_qu_il_n_y_a_pas_de_recouvrement(p,d,n):
    for i in range(n):
        y=616*(n-i)+8*d+72
        a=2**i
        z=y%(p*a)
...
def cherche_d_evitant_un_recouvrement_exhaustif(p,a,nb=1000):
    b=math.log2(a)
    if b==0:
        n=1
    if b%1==0:
        ...
        n=int(b)+1
    for d in range(nb):
        if not check_qu_il_n_y_a_pas_de_recouvrement(p,d,n):
            return d
    return -20
...
def plus_petit_facteur_suffisament_grand(p):
    if p<200:
        return False
    for multiplier in range(p//200):
        multiplier=p//200-multiplier
        if p%multiplier==0:
            return multiplier

"""canon_booste_max qui utilise period_multiplier pour créer un oscillateur de période p, a et d pouvant être donné, par défaut a et d sont obtenus avec divise puis cherche"""

def canon_booste_max(p,a=-1,d=-1):
    if p<69:
    ...
    haut_gauche,r=multiplicateur_de_periode_active(l,haut_gauche,r,a,d)
    return l

"""identique à canon_booste_max, mais un eater est ajouté à la fin, transformant le canon en oscillateur"""

def oscillateur_booste_max(p,a=-1,d=-1):
    if p<61:
...
"""aire_naive prend en entrée un entier p et renvoie la taille de l'oscillateur naif qui lui est associé"""

def aire_naive(p):
    a,k1,k2,k3,A,B=trouve_les_plus_petites_constantes_exhaustif(p)
    x,y=taille_de_oscillateur_de_constantes(k1,k2,k3,A,B)
    return x,y

"""aire_booste_max prend en entrée un entier p, possiblement un entier a par lequel multiplier p, par défaut on multiplie p//a par a avec a=plus_petit_facteur_suffisament_grand(p), un booleen Id qui indique si le d du period_multiplier doit également être renvoyé, des aractéristiques d'oscillateur k1,...,B, qui sont par défaut obtenues par un appel à plus_petit, et enfin d, qui est par défaut obtenu par cherche. aire_booste_max renvoie alors la taille de l'oscillateur_booste_max obtenu avec ces caracteristiques"""

def aire_booste_max(p,a=-1,Id=False,k1=-1,k2=-1,k3=-1,A=-1,B=-1,d=-1):
    if p<69:
        print("oops, la periode doit etre supperieure a 69!")
        return 0
    if p<300:
    ...
    if d==-1:
        d=cherche_d_evitant_un_recouvrement_exhaustif(i,a)
    if k1==-1:
        nimporte,k1,k2,k3,A,B=trouve_les_plus_petites_constantes_exhaustif(i)
    x,y=taille_de_oscillateur_de_constantes(k1,k2,k3,A,B)
...
"""recherche_exhautsive_de_constantes_pour_optimal doit trouver pour p un entier une periode apparente dans dico p_app, un entier multiplier et un entier ajout<=50 tel que p=delta+multiplier*p_app, avec delta qui vaut 1042+8*ajout si p est pair (car p_app est toujours pair) et delta qui vaut 1467+8*ajout si p est impair.
...
def recherche_exhautsive_de_constantes_pour_optimal(p):
...
def canon_optimal(p):
    if p>=2000:
        ...
        r = "droit"
        liste=creer_liste_pour_oscillateur_naif(k1,k2,k3,A,B)
        n=len(liste)


        for x in range(2):

            haut_gauche,r=positionner_puis_faire_avancer_une_figure(l,haut_gauche,r,conduith_to_g)
            if p%2==0:
                haut_gauche,r=positionner_puis_faire_avancer_une_figure(l,haut_gauche,r,conduitmega_bistable)
            else:
                haut_gauche,r=positionner_puis_faire_avancer_une_figure(l,haut_gauche,r,conduitmega_bistable_decale)
            haut_gauche=deplace_un_planeur(haut_gauche,r,50)
            haut_gauche,r=positionner_puis_faire_avancer_une_figure(l,haut_gauche,r,conduitsyringe)
            ...
            for i in range(n//2):
                j=liste[x*n//2+i]
                haut_gauche,r=positionner_puis_faire_avancer_une_figure(l,haut_gauche,r,liste_de_conduit[j])
        ...
        haut_gauche_glider=deplace_un_planeur(haut_gauche_glider,r,10)
        haut_gauche_glider,r=positionner_puis_faire_avancer_une_figure(l,haut_gauche_glider,r,conduitsnarkrenverse)
        haut_gauche_glider=deplace_un_planeur(haut_gauche_glider,r,78)
        haut_gauche_glider,r=positionner_puis_faire_avancer_une_figure(l,haut_gauche_glider,r,conduitsnark)
        haut_gauche_glider=deplace_un_planeur(haut_gauche_glider,r,119)
        ticks=1180+127+560+4*50

        haut_gauche_glider,r=multiplicateur_de_periode_active(l,haut_gauche_glider,r,multiplier,d)
        ...
        if le_suivant<200 or le_suivant>periode_apparente-80 :
            if ajout<37:
                haut_gauche_glider=deplace_un_planeur(haut_gauche_glider,r,25)
            else:
                haut_gauche_glider=deplace_un_planeur(haut_gauche_glider,r,45)
            haut_gauche_glider,r=positionner_puis_faire_avancer_une_figure(l,haut_gauche_glider,r,conduitsnarkrenverse)
            haut_gauche_glider=deplace_un_planeur(haut_gauche_glider,r,5)
            haut_gauche_glider,r=positionner_puis_faire_avancer_une_figure(l,haut_gauche_glider,r,conduitsnark)
            haut_gauche_glider,r=positionner_puis_faire_avancer_une_figure(l,haut_gauche_glider,r,conduitsnark)
            haut_gauche_glider,r=positionner_puis_faire_avancer_une_figure(l,haut_gauche_glider,r,conduitsnarkrenverse)

        haut_gauche,r=haut_gauche_ressortissant,"gauche"
        haut_gauche=deplace_un_planeur(haut_gauche,r,ajout)
        if p%2==1:
            haut_gauche,r=positionner_puis_faire_avancer_une_figure(l,haut_gauche,r,conduitsnarkrenverse)
            haut_gauche=deplace_un_planeur(haut_gauche,r,29)
            haut_gauche,r=positionner_puis_faire_avancer_une_figure(l,haut_gauche,r,conduitsyringe)
            haut_gauche,r=positionner_puis_faire_avancer_une_figure(l,haut_gauche,r,conduith_to_g)
            haut_gauche,r=positionner_puis_faire_avancer_une_figure(l,haut_gauche,r,conduitsnark)
            haut_gauche,r=positionner_puis_faire_avancer_une_figure(l,haut_gauche,r,conduitsnarkrenverse)
            haut_gauche,r=positionner_puis_faire_avancer_une_figure(l,haut_gauche,r,conduitsnarkrenverse)
            haut_gauche=deplace_un_planeur(haut_gauche,r,16)
            haut_gauche,r=positionner_puis_faire_avancer_une_figure(l,haut_gauche,r,conduitsnark)
            haut_gauche=deplace_un_planeur(haut_gauche,r,-5)
            haut_gauche,r=positionner_puis_faire_avancer_une_figure(l,haut_gauche,r,conduitsnarkrenverse)
        if p%2==0:
            haut_gauche,r=positionner_puis_faire_avancer_une_figure(l,haut_gauche,r,conduitsnarkrenverse)
            haut_gauche=deplace_un_planeur(haut_gauche,r,63)
            haut_gauche,r=positionner_puis_faire_avancer_une_figure(l,haut_gauche,r,conduitsnarkrenverse)
        return l
    else:
        return canon_booste_max(p)
...
def oscillateur_optimal(p,suivi=False):
    if p>=2000:
        ...
        for x in range(2):
            haut_gauche,r=positionner_puis_faire_avancer_une_figure(l,haut_gauche,r,conduith_to_g)
            if p%2==0:
                haut_gauche,r=positionner_puis_faire_avancer_une_figure(l,haut_gauche,r,conduitmega_bistable_sans_sortie)
            else:
                haut_gauche,r=positionner_puis_faire_avancer_une_figure(l,haut_gauche,r,conduitmega_bistable_decale_sans_sortie)
            haut_gauche=deplace_un_planeur(haut_gauche,r,50)
            haut_gauche,r=positionner_puis_faire_avancer_une_figure(l,haut_gauche,r,conduitsyringe)
            if x==0 :
                for i in range(len(herschel)):
...
"""aire_optimale prend en entrée un entier p et x dans le format de recherche_exhautsive_de_constantes_pour_optimal(p) associé à p, et renvoie une évaluation de la taille de de l'oscillateur fourni par la solution optimale associé à x """

def aire_optimale(p,x):
    if p<2000:
        return 1,1
    if x==False:
        if p>=10**5:
...
"""aire_max_a_peu_pres donne une borne de la taille de l'oscillateur de la solution boostée en considérant que d est plus petit que 50, ce qui est empiriquement vérifié"""

def aire_max_a_peu_pres(p,a=-1,k1=-1,k2=-1,k3=-1,A=-1,B=-1):
    if p<69:
        print("oops, la periode doit etre supperieure a 69!")
        return 0
    if p<245:
...
"""aire_optimale_a_peu_pres agit de même, en utilisant aire_max_a_peu_pres"""
def aire_optimale_a_peu_pres(p,x):
    if p<2000:
        return 1,1
    if x==False:
        if p>=10**5:
    ...
    if p%2==0:
        ...
        periode_apparente=(p-1367-8*ajout)//multiplier
    x,y=aire_max_a_peu_pres(periode_apparente,multiplier,k1,k2,k3,A,B)
    x,y=x+400,y+300
    return x,y
...
def matrix_a_rle(l1, en_tete=True, fin=True):
    ...
    for i, row in enumerate(l[:-1]):
        ...
        if compteur>100:
            r.append("\n")
            compteur=0
    r.append(ligne_a_rle(l[-1]))
    r.append('!' if fin else '$')
    return ''.join(r)

def enregistre_la_grille(l1,name):
    ...
    def f(i,j):
        if i<=1 or j<=1 or i>n+1 or j>m+1:
            return (0.,0.,0.)
        return (1-float(l[i-2][j-2]),1-float(l[i-2][j-2]),1-float(l[i-2][j-2]))
    image=[[f(i//4,j//4) for j in range(4*(m+4))] for i in range(4*(n+4))]
